...
public class JavaWarmUp {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (category[i].equals("laptop") ){
                catIndex = 1;}
            if (category[i].equals("smart_watch") ){
                catIndex = 2;}
        ...
        for (int j = 0; j < categoriesList.length; j++) {
            if (numOfBatchesC[j] > 0) {
                ...
                System.out.println(String.format("%.2f",(totFeeC[j]-totAsmCostC[j]-totLaborCostC[j])/totQuantityC[j]));
    ...
    static int getMaxPriceIndex(double[] priceT){
        ...
        return(counter); // modify this as well
        // Your code ends here.
