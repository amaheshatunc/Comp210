...
public class LinkedList<T> {
    ...
    public void removeAtIndex(int i) {
        validIndex(i); // First edge case (uncommon case).
        if (isEmpty()) { // Second edge case.
            return;
        }
        Node<T> current = head; // Starts at Head.
        for (int j = 0; j < i; j++) { // Keep going through the loop till you find the thing you want.
    ...
    public boolean isEqual(LinkedList list2) {
        if (size != list2.size()) { // Edge Case 1
            return false;
        }
        for (int i = 0; i < size; i++) {
            if (!(get(i).equals(list2.get(i)))){ // This checks if at least 1 element in the list is not equal to another element in  the other list.
                return false;
            }
        }
        return true;
